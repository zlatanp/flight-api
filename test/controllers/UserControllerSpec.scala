...
class UserControllerSpec extends PlaySpec with GuiceOneAppPerTest with Injecting {
  ...
  "HomeController GET" should {
    ...
    "response for not connected user" in {
      val controller = new UserController(stubControllerComponents(), db)
    ...
    "response for LOG IN endpoint" in {
      ...
      contentType(home) mustBe Some("application/json")
      contentAsString(home) must be("[{\"success\":\"login\"},{\"user\":{\"id\":1,\"firstName\":\"A\",\"lastName\":\"AA\",\"name\":\"aaa\",\"password\":\"xxx\",\"salt\":\"x1x1x1\",\"typeOfUser\":\"Admin\"}}]")
    }
